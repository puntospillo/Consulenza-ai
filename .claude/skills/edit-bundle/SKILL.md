...
- `Jle2` = vista "Statistiche progetti interni" (Consulenza), raggiungibile con un toggle dentro `Vle` (Statistiche & BI) — stesso menu, vista separata. Mostra ore/spese/costo tecnico totali, grafico ore per progetto interno, grafico utilizzo modelli AI (`aiModel` delle time-entries) e tabella dettaglio. `Vle` di default mostra ancora la vista "esterni" originale, invariata.

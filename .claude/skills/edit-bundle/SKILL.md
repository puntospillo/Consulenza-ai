...
**BUG CORRETTO il 02/07/2026 — "Salva ora" azzerava lo stato**: in `Wle` il salvataggio manuale faceva `let{ts:n,merged:r}=await ZJ(...)` ma `ZJ` ritorna una STRINGA (timestamp), quindi `r` era `undefined` e `t(e=>({...r,...}))` sostituiva lo stato con un oggetto quasi vuoto. Se si cambia la firma di ritorno di `ZJ`/`QJ`, controllare TUTTI i call site (il destructuring su una stringa non dà errore, fallisce in silenzio).
...
**"Da verificare" (campo dati `sospeso`) NON è più uno stato ma un flag booleano** (cambiato 01/07/2026 su richiesta esplicita: "metti un bottone di fianco allo stato invece di metterlo nello stato"): campo `sospeso` sull'intervento, ortogonale a `stato` (che ora è solo da_assegnare/aperto/in_lavorazione/chiuso — `AI` non contiene più `sospeso`). `II()` migra i vecchi record `stato==='sospeso'` → `stato:'aperto', sospeso:true` (idem l'import Excel). UI: in `cle` un bottone toggle "Sospeso" di fianco al select Stato; in `mle` badge violet "Sospeso" accanto al badge stato e il filtro rapido `sospeso` filtra sul flag; in `F9` (Report/Statistiche) campo `sosp` + bottone "Solo sospesi"; in dashboard `ule` la casella "Da verificare" = `e.sospeso` (TUTTI i flaggati, anche i chiusi — il filtro `stato!==chiuso` è stato rimosso il 02/07/2026 perché nascondeva i chiusi da verificare); `O9` (rapportino) stampa " · DA VERIFICARE". **Etichetta UI: sempre "Da verificare"** (rinominata il 02/07/2026, prima era "Sospeso") — il campo dati resta `sospeso`, non rinominarlo. Se si aggiunge una nuova vista con filtro stato, ricordarsi che sospeso va gestito a parte.
...
- **FIX 03/07/2026 — anche `QI()` (usata dalla pagina Fornitori `_le`) aveva lo stesso bug di `Ele`**: calcolava il costo per fornitore SEMPRE su `oreImpiegate*tecnico.costoOrario` (tariffa standard), ignorando `costoTecnicoOrario` una volta registrata la fattura. Risultato: margine diverso tra pagina Fornitori e Report per lo stesso fornitore (segnalato dall'utente su un fornitore reale, "MPF": 20% su Fornitori vs 34% su Report). Corretto sostituendo il calcolo locale con `ZI(e,t)` dentro il loop di aggregazione, coerente con `Ele`/Report/Statistiche. **Ogni vista che aggrega costo per intervento deve usare `ZI()`, mai un calcolo `oreImpiegate*costoOrario` locale** — quel pattern ignora sistematicamente l'override da fattura fornitore.

**FIX 03/07/2026 — Ele (Fatture Fornitori) non rispettava il costo da fattura nella propria vista**: `ZI()` già usava `costoTecnicoOrario` (override scritto da "Segna come fatturati") ovunque nell'app da tempo (mle/ble/xle/ple), MA `Ele` calcolava "Costo stimato" e Margine/Margine% con una funzione locale `x(e)` che leggeva SEMPRE il costo standard del tecnico, ignorando l'override. Corretto sostituendo `x(t)` con `ZI(t,i)` nelle due celle. `x(e)` resta usata SOLO per precompilare il campo editabile "Importo fatturato" quando non c'è ancora un valore salvato (stima iniziale pre-fattura, corretto che sia lo standard).

**Fatture Fornitori (`Ele`) — filtri cliente/contratto/tecnico + "Solo da verificare"** (03/07/2026): stati `cli`/`ctr`/`tec`, applicati sopra il risultato di `vL()` (che già filtra fornitore/periodo/stato fattura). Il select contratto si restringe ai contratti del cliente selezionato (si azzera se cambi cliente); il select tecnico usa `iForn` = tecnici non interni con fornitoreId/tipologia fornitore (stesso criterio di esclusione tecnici interni usato altrove). Bottone toggle `sosp` (stesso pattern di `F9`) filtra sul flag `sospeso` (etichetta UI "Da verificare", vedi nota sopra).
...
**Zebra striping globale (02/07/2026, richiesta "maggior contrasto fra le righe")**: regole CSS aggiunte in coda al `<style>` compilato: `tbody tr:nth-child(odd/even)` (grigio chiaro #f8fafc / scuro #e2e8f0) valgono per TUTTE le tabelle. Le due tabelle con righe appaiate (riga dati + riga descrizione: `mle` e `ble`) usano invece le classi dedicate `zA`/`zB` (stesso colore per la coppia, indice del `.map`) che vincono sull'nth-child perché definite dopo a pari specificità; `zHov` dà l'hover azzurro sulle righe cliccabili di `mle`; `tbody tr.bg-emerald-50\/40` ripristina l'evidenza selezione in `Ele`. Se si aggiunge una nuova tabella con righe multiple per record, usare `zA`/`zB` con l'indice, altrimenti l'nth-child spezza la coppia. Lo sfondo pagina è `bg-slate-200` (era slate-50) per far risaltare le caselle bianche; i menu laterali alternano `bg-slate-50`/`bg-slate-200` sulle voci non attive (indice `mi` nei `.map` di `R` e `oe`).

**Dashboard Assistenza — bottone "Nuovo intervento"** (02/07/2026): in alto a destra in `ule` (prop `onNew`), accanto a "contratti esauriti". In `jle` fa `p(`__new__`)` + naviga a `a_interventi`: `mle` non trova nessun intervento con quell'id → `{initial:null}` → si apre il modal di NUOVO intervento. Meccanismo volutamente riusato da `focusId`, nessuno stato nuovo.

**Report Consulenza (`Hle`) — bottone "Escludi contratti"** (02/07/2026): stato locale `noCtr`, filtra `billingStatus!=='contratto'` a monte del memo `x` (quindi vale anche per totali, CSV e `printNow`); aggiunge il chip "Contratti esclusi" nei parametri. Ricordarsi `noCtr` nelle dependency dei due useMemo.

**Icone app (favicon/apple-touch-icon) — file separati, MAI data: URI** (03/07/2026, bug reale: icona visibile su Chrome ma non su Safari/iPhone): Safari e iOS non supportano in modo affidabile le icone codificate come `data:` URI base64 nell'`<head>` (né favicon SVG né soprattutto `apple-touch-icon` per "Aggiungi a Home"), anche se il tag è sintatticamente corretto — Chrome invece le mostra comunque, il che rende il problema invisibile finché qualcuno non prova da iPhone/Safari. Le icone ora sono file reali accanto a `index.html`: `favicon.svg`, `icon-16.png`, `icon-32.png`, `apple-touch-icon.png` (180×180, standard Apple), `icon-192.png`/`icon-512.png` (per `manifest.json`, usato anche da Android/Chrome per "Aggiungi a Home"). Questi NON sono dentro l'unico bundle `index.html`: sono file binari a parte nella root del repo, vanno committati insieme a `index.html` e pubblicati su GitHub Pages allo stesso modo. Se si rigenera l'icona (nuovo logo), rifare tutte le dimensioni con Pillow (`Image.resize`) e mantenere gli stessi nomi file referenziati nell'head.

**BUG CRITICO CORRETTO il 18/09/2026 — Report Consulenza (`Hle`) andava in schermata bianca/crash quando i filtri non trovavano nulla**: nel blocco "Nessuna voce per i filtri selezionati" c'era `(0,X.jsx)(F,{size:32,...})` dove `F` era in realtà la variabile locale del filtro condiviso `Vle2` (`let F=Vle2({...})`, usata ovunque come `F.billing`/`F.client`/`F.controls`/ecc.), NON l'icona globale `F` (usata altrove, es. in `Ile`/todos, per il check "fatto/non fatto"). React tentava di renderizzare un oggetto come componente → crash totale della pagina (React error #130), MA SOLO quando `x.length===0` (nessun risultato) perché è l'UNICO punto in cui `F` veniva usato come componente invece che come oggetto filtri — tutti gli altri ~60 usi di `F` nella funzione (`F.xxx`) non generano errore a prescindere da cosa sia `F`, quindi il bug è rimasto invisibile per mesi. Fix: rinominata la variabile locale del filtro in `Flt` in tutta la funzione `Hle` (regex `\bF\b` sull'intero corpo funzione, non un replace_unique puntuale — 61 occorrenze), poi ripristinato `F` (l'icona globale corretta) nel singolo punto del componente icona vuoto. **Lezione generale**: quando si introduce un hook/variabile condivisa con un nome a lettera singola (`Vle2` → `F`, `K3`, ecc.) in una funzione già esistente, verificare che quella lettera non sia già un'icona globale usata più avanti nello stesso corpo funzione — cercare `(0,X.jsx)(LETTERA,` prima di scegliere il nome. Il test jsdom va sempre eseguito anche con **dataset vuoto** (`x.length===0`/liste vuote), non solo con dati di esempio: molti bug di questo tipo si manifestano solo nel ramo "nessun risultato".

**Guardie "nessun dato da stampare" aggiunte il 18/09/2026** (richiesta: "se non ci sono dati va su una pagina bianca e si blocca"): `Hle` e `ble` (`printNow`) ora controllano `x.length===0`/`n.length===0` a inizio funzione e mostrano un `alert()` invece di aprire il popup — evita l'esperienza confusa di un popup quasi vuoto con il dialogo di stampa del browser sopra. `lle` e `k9` avevano già una riga fallback "Nessun intervento" dentro la tabella stampata, quindi non necessitavano della stessa guardia.

**Escaping HTML nei template di stampa esteso a tutti i campi liberi (18/09/2026)**: `descrizione`/`motivoRichiesta` (parzialmente) erano già escapati con `.replace(/</g,\`&lt;\`)` prima di finire nell'HTML via `document.write`, ma nome cliente, nome tecnico, N° ticket, N° fattura fornitore e nome progetto NON lo erano in nessuno dei 5 template (`lle`,`O9`,`k9`,`ble` printNow,`Hle` printNow) — dato che questi popup ereditano l'origin dell'app (aperti con `window.open('','_blank')`+`document.write`, non navigati altrove), uno di questi campi con `<script>`/`<img onerror=...>` avrebbe eseguito codice con accesso a `localStorage` (dove MSAL tiene la cache del login) nella sessione di chi stampa/apre quel record — un vettore di furto sessione realistico, non solo un difetto estetico. Ogni nuovo campo di testo libero aggiunto a un template di stampa deve SEMPRE passare da `.replace(/</g,\`&lt;\`)` (o equivalente) prima di finire in una stringa HTML.

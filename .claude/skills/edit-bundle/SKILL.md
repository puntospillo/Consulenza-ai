...
**Fatture Fornitori (`Ele`) — filtri cliente/contratto/tecnico** (03/07/2026): stati `cli`/`ctr`/`tec`, applicati sopra il risultato di `vL()` (che già filtra fornitore/periodo/stato fattura). Il select contratto si restringe ai contratti del cliente selezionato (si azzera se cambi cliente); il select tecnico usa `iForn` = tecnici non interni con fornitoreId/tipologia fornitore (stesso criterio di esclusione tecnici interni usato altrove).

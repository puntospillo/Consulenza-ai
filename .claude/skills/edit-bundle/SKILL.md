...
- **Allegati PDF sull'intervento** (aggiunto 22/06/2026, campo `allegati:[{id,name,webUrl,size}]`): caricati su OneDrive via Graph API con `uploadAllegato(state.cloud, interventoId, file)` / rimossi con `deleteAllegato(state.cloud, fileId)` (vicino a `YJ`/`XJ`/`ZJ`/`QJ` nel bundle). **Lo stato globale ha già tutto il necessario per chiamare Graph da qualsiasi componente**: `state.cloud` = `{clientId,tenantId,filePath}` (inizializzato da `NI`, salvato nello stesso JSON dell'app) — quindi un componente che riceve `state:e` come prop può fare `KJ(e.cloud)` per ottenere un token senza bisogno di prop-drilling aggiuntivo per l'autenticazione. La cartella allegati è `<cartella del filePath>/Allegati/<id>/`, sorella della cartella che contiene `consulenza-ai-stato.json`. Upload semplice via `PUT .../content` (limite Graph ~4MB, niente upload-session a chunk) — coerente con come l'app salva già il JSON di stato, ma da tenere a mente se un domani servissero PDF più grandi.
